...
def compute_confusion_angle(uv_list):
    """
    Compute the confusion angle using PCA on adjacency (error) vectors
    in CIE 1976 u′v′ space, adjusted to the standard −90° to +90° range.
    This method takes into account the order in which the caps are arranged.
    """
    diffs = []
    for i in range(len(uv_list) - 1):
        du = uv_list[i+1][0] - uv_list[i][0]
...
# def compute_confusion_angle(uv_list):
#     """
#     Compute the confusion angle using PCA on the adjacency (error) vectors,
#     i.e. the differences between successive u′, v′ coordinates.
#     This method takes into account the order in which the caps are arranged.
#     """
#     # Calculate the differences (error vectors) between consecutive points.
#     diffs = []
#     for i in range(len(uv_list) - 1):
#         du = uv_list[i+1][0] - uv_list[i][0]
#         dv = uv_list[i+1][1] - uv_list[i][1]
...
#     if not diffs:
#         return 0.0  # Not enough data to compute an angle.
    
#     diffs = np.array(diffs)
#     # Center the differences for PCA.
#     diffs_centered = diffs - np.mean(diffs, axis=0)
...
# =====================================
# 1) DEFINE A TARGET NORMAL TES, e.g. 16
...
def submit():
    ...
    scaled_tes = scale_factor * tes  # scale TES
